function [PBC_q, PBC_lambda] = PBC_fun(a, b, c, d, dr, ds, k__0, k__1, k__2, flag)
    ...
    i = 1;
    for q = -pi:0.1:pi
        PBC_lambda(:,i) = double(solve(det(subs(A)) == 0, lambda));
        PBC_q(i) = q;
        i = i + 1;
    end
    ...
    figure
    for i = 1:max(size(PBC_lambda(:,1)))
        plot(real(PBC_lambda(i, :)), imag(PBC_lambda(i, :)))
        hold on
    end
